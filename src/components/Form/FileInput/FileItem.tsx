...
import { fomartBytes } from '@/utils/format-bytes'
...
const fileItem = tv({
  ...
  variants: {
    state: {
      ...
      error: {
        container:
          'bg-error-25 border-error-300 dark:border-error-500/30 dark:bg-error-500/5',
        icon: 'border-error-50 bg-error-100 text-error-600 dark:bg-error-500/5 dark:border-error-500/30 dark:text-error-400',
        deleteButton:
          'text-error-700 hover:text-hover:900 dark:text-error-400 dark:hover:text-error-300',
      },
    },
  },
...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  return (
    <div className={container()}>
      ...
      {state === 'error' ? (
        ...
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <div className="flex w-full items-center gap-3">
            ...
            <span className="-translate-y-0.5 text-sm font-medium text-zinc-700">
      ...
      {state === 'complete' ? (
        <CheckCircle2 className="h-5 w-5 fill-violet-600 text-white dark:text-zinc-400" />
      ) : (
